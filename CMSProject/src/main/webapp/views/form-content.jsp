...
<body>
	...
	<div id="main-content">
		...
		<c:if test="${content == null}">
			<form action="insertContent" class="form" id="form-content"
				method="post">
				<div id="sub-title">Add Content Elements</div>
		</c:if>

		<c:if test="${content != null}">
			<form action="updateContent" class="form" id="form-content"
				method="post">
		...
		<div id="input-content">
			...
			<div class="item">
				<label for="title" class="form-label">Title</label> <input
					...
					value="<c:out value='${content.title}' />" /> <span
					class="form-message"></span>
			</div>
			<!-- Input Brief -->
			<div class="item">
				<label for="brief" class="form-label">Brief</label>
				<textarea name="brief" id="brief" class="form-control" rows="2"><c:out value='${content.brief}' /></textarea>
				<span class="form-message"></span>
			</div>
			<!-- Input Content -->
			<div class="item">
				<label for="content" class="form-label">Content</label>
				<textarea name="content" id="content" class="form-control" rows="4"><c:out value='${content.content}' /></textarea>
				<span class="form-message"></span>
			</div>
			<!-- Submit -->
